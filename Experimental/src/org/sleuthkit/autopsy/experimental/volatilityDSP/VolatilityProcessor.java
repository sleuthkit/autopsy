...
class VolatilityProcessor {
    ...
    boolean run() {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Failed to get the current case", ex);
            return true;
        }
        ...
        moduleOutputPath = Paths.get(currentCase.getModuleDirectory(), VOLATILITY_DIRECTORY, dataSourceId.toString()).toString();
        ...
        for (String file : fileSet) {
            ...
            try {
                ...
                if (resolvedFiles.isEmpty()) {
                    logger.log(Level.SEVERE, "File not found in lookup: {0}/{1}", new Object[]{filePath, fileName});
    ...
    /**
     * Normalize the path we parse out of the output before we look it up in the
     * case DB
     *
    ...
    private String normalizePath(String filePath) {
        ...
        // strip C: and \??\C:
        if (path.contains(":")) {
            path = path.substring(filePath.indexOf(':') + 1);
        }
        ...
        if (path.contains("/harddiskvolume")) {
            // 16 advances beyond harddiskvolume and the number
            path = path.substring(path.indexOf("/harddiskvolume") + 16);
        }

        // no point returning these. We won't map to them
        if (path.startsWith("/namedpipe/")) {
            return "";
        }
