...
final class AutoIngestJobLogger {
     ...
     * @param systemLogger       The auto ingest system logger, used to log
     *                           errors encountered by this logger.
     */
    AutoIngestJobLogger(Path manifestPath, String dataSourceFileName, Path caseDirectoryPath, Logger sysLogger) {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logFailedToExtractDataSource() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logFailedToParseLogicalReportDataSource() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logDataSourceProcessorCancelled() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logDataSourceProcessorError(String dsp) throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logDataSourceAdded() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logFailedToAddDataSource() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logNoDataSourceContent() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logIngestModuleStartupErrors() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logAnalysisStartupError() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logAnalysisCancelled() throws InterruptedException {
     ...
     *                              acquire an exclusive lock on the log file.
     */
    void logFileExportError() throws InterruptedException {
    ...
    private void log(MessageCategory category, String message) throws InterruptedException {
        ...
        try (Lock lock = CoordinationService.getInstance().tryGetExclusiveLock(CoordinationService.CategoryNode.CASES, logPath.toString(), LOCK_TIME_OUT, LOCK_TIME_OUT_UNIT)) {
            if (null != lock) {
                ...
                sysLogger.log(Level.SEVERE, String.format("Failed to write case auto ingest log message (\"%s\") for %s due to time out acquiring log lock", message, manifestPath));
            }
        } catch (CoordinationServiceException | IOException ex) {
            sysLogger.log(Level.SEVERE, String.format("Failed to write case auto ingest log message (\"%s\") for %s", message, manifestPath), ex);
        }
    }
