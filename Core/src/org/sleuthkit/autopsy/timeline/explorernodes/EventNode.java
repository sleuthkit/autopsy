...
public class EventNode extends DisplayableItemNode {
    ...
    EventNode(TimelineEvent event, Content file, BlackboardArtifact artifact) {
        ...
        this.event = event;
        //TODO: filesystem events get the icon of the first type encountered...
        EventType evenType = event.getEventTypes().stream().findFirst().orElseThrow(()
                -> new NoSuchElementException("Event has no type. " + event.getEventID() + ": " + event.getFullDescription()));
    ...
    protected Sheet createSheet() {
        ...
        properties.put(new NodeProperty<>("eventSubType", Bundle.NodeProperty_displayName_subType(), "sub type",
                event.getEventTypes().stream().map(EventType::getDisplayName).collect(joining(", ")))); // NON-NLS
