...
class NewCaseWizardPanel1 implements WizardDescriptor.ValidatingPanel<WizardDescriptor> {
    ...
    public void readSettings(WizardDescriptor settings) {
        ...
        String createdDirectoryLocal = (String) settings.getProperty("createdDirectory");
        if(createdDirectoryLocal != null && !createdDirectoryLocal.equals("")) {
            logger.log(Level.INFO, "Deleting a case dir in readSettings(): " + createdDirectoryLocal);
            Case.deleteCaseDirectory(new File(createdDirectoryLocal));
        }
    }
