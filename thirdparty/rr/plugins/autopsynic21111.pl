...
#
# References
#   LeaseObtainedTime - httptechnet.microsoft.comen-uslibrarycc978465.aspx
#   T1 - httptechnet.microsoft.comen-uslibrarycc978470.aspx
# 
...
sub pluginmain {
	...
	#rptMsg((.getHive().) .getShortDescr().n); # banner
	my $reg = ParseWin32Registry-new($hive);
...
# file
	...
	my @nics;
	my $key_path = ControlSet00.$current.ServicesTcpipParametersInterfaces;
	my $key;
			...
			foreach my $g (@guids) {
				...
				eval {
					...
					foreach my $v (@vals) {
						...
						$data = gmtime($data). Z if ($name =~ mTime$);
				...
				if (scalar @ssids  0) {
					foreach my $ssid (@ssids) {
						...
						::rptMsg("<ssid name=\"SSIDName\">". $ssid_realname ."</ssid>");
