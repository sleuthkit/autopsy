...
sub pluginmain {
	my $class = shift;
	my $ntuser = shift;
	::logMsg("Launching appx v.".$VERSION);
	::rptMsg("appx v.".$VERSION); 
		...
		if (scalar @subkeys > 0) {
			foreach my $sk (@subkeys) {
				...
				eval {
					...
					::rptMsg($key_path."\\".$name."  LastWrite Time: ".::format8601Date($lw)."Z");
					...
					if (scalar @sk2 > 0) {
						foreach my $s2 (@sk2) {
							eval {
								...
								::rptMsg($key_path."\\".$key_path2."\\".$name."  LastWrite time: ".::format8601Date($lw)."Z");
								::rptMsg("DebugPath value: ".$debug);
							};
						}
					}
				}
