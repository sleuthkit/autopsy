...
@REM set "EXAMPLE_DIR=%SOLR_TIP%\example"
@REM set TMP=!SOLR_HOME:%EXAMPLE_DIR%=!
@REM IF NOT "%TMP%"=="%SOLR_HOME%" (
@REM   set "SOLR_LOGS_DIR=%SOLR_HOME%\..\logs"
@REM   set "LOG4J_CONFIG=file:%EXAMPLE_DIR%\resources\log4j.properties"
@REM )
...
IF "%FG%"=="1" (
  ...
  "%JAVA%" %SERVEROPT% %SOLR_JAVA_MEM% %START_OPTS% %GCLOG_OPT%:"!SOLR_LOGS_DIR!"/solr_gc.log -Dlog4j.configuration="%LOG4J_CONFIG%" -DSTOP.PORT=!STOP_PORT! -DSTOP.KEY=%STOP_KEY% ^
    -Djetty.port=%SOLR_PORT% -Dsolr.solr.home="%SOLR_HOME%" -Dsolr.install.dir="%SOLR_TIP%" -Djetty.home="%SOLR_SERVER_DIR%" -Djava.io.tmpdir="%SOLR_LOGS_DIR%\tmp" -jar start.jar "%SOLR_JETTY_CONFIG%"
) ELSE (
  START /B "Solr-%SOLR_PORT%" /D "%SOLR_SERVER_DIR%" "%JAVA%" %SERVEROPT% %SOLR_JAVA_MEM% %START_OPTS% %GCLOG_OPT%:"!SOLR_LOGS_DIR!"/solr_gc.log -Dlog4j.configuration="%LOG4J_CONFIG%" -DSTOP.PORT=!STOP_PORT! -DSTOP.KEY=%STOP_KEY% ^
    -Djetty.port=%SOLR_PORT% -Dsolr.solr.home="%SOLR_HOME%" -Dsolr.install.dir="%SOLR_TIP%" -Djetty.home="%SOLR_SERVER_DIR%" -Djava.io.tmpdir="%SOLR_LOGS_DIR%\tmp" -jar start.jar "%SOLR_JETTY_CONFIG%" > "!SOLR_LOGS_DIR!\solr-%SOLR_PORT%-console.log"
  echo %SOLR_PORT%>"%SOLR_LOGS_DIR%"\solr-%SOLR_PORT%.port
...
"%JAVA%" -version 2>&1 | findstr /i "version" > %TEMP%\javavers
set /p JAVAVEROUT=<%TEMP%\javavers
del javavers
...
"%JAVA%" -version 2>&1 | findstr /i "IBM J9" > %TEMP%\javares
set /p JAVA_VENDOR_OUT=<%TEMP%\javares
del javares
