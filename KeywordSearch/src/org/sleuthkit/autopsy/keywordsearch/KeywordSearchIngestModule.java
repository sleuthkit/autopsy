...
//only search images from current ingest, not images previously ingested/indexed
    ...
    private boolean extractStringsAndIndex(AbstractFile aFile) {
        try {
            ...
            if (Ingester.getDefault().search(extractedTextReader, aFile.getId(), aFile.getName(), aFile, KeywordSearchIngestModule.this.context, false, settings.isIndexToSolrEnabled(), settings.getNamesOfEnabledKeyWordLists()) ) {
                if(!settings.isIndexToSolrEnabled()) {
                    Ingester.getDefault().indexFile(KeywordSearchUtil.getReader(aFile, stringsExtractionContext), aFile.getId(), aFile.getName(), aFile, KeywordSearchIngestModule.this.context, false);
                }
    ...
    private boolean searchTextFile(AbstractFile aFile) {
        try {
            ...
            } else if (Ingester.getDefault().search(textReader, aFile.getId(), aFile.getName(), aFile, context, true, settings.isIndexToSolrEnabled(), settings.getNamesOfEnabledKeyWordLists()) ) {
                if(!settings.isIndexToSolrEnabled()) {
                    Ingester.getDefault().indexFile(new TextFileExtractor(aFile).getReader(), aFile.getId(), aFile.getName(), aFile, KeywordSearchIngestModule.this.context, false);
                }
                textReader.close();
                putIngestStatus(jobId, aFile.getId(), IngestStatus.TEXT_INGESTED);
                return true;
