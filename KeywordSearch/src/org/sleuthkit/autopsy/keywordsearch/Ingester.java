...
//    }
     ...
     */
    // TODO (JIRA-3118): Cancelled text indexing does not propagate cancellation to clients 
    < T extends SleuthkitVisitableItem> boolean search(Reader sourceReader, long sourceID, String sourceName, T source, IngestJobContext context, boolean doLanguageDetection, boolean indexIntoSolr, List<String> keywordListNames) throws Ingester.IngesterException, IOException, TskCoreException, Exception {
        ...
        try (BufferedReader reader = new BufferedReader(sourceReader)) {
            ...
            while (chunker.hasNext()) {
                ...
                if(keywordListNames != null) {
                    boolean result = searcher.searchChunk(chunk, sourceID, numChunks);
                    if(result) {
                        hitFound = result;
                    }
                }
    ...
    private void indexWholeFile(Reader sourceReader, IngestJobContext context, long sourceID, String sourceName, Optional<Language> language, Map<String, String> contentFields) throws IngesterException, IOException {
        InputStream targetStream = new ReaderInputStream(sourceReader);

        try (BufferedReader reader = new BufferedReader(new InputStreamReader(targetStream))) {
            Chunker chunker = new Chunker(reader);
            int numChunks = 0;
            ...
            while (chunker.hasNext()) {
                ...
                Chunk chunk = chunker.next();
                indexChunk(chunk, sourceID, sourceName, numChunks + 1, language, contentFields, chunker.hasNext());
                numChunks++;
            }
            if (chunker.hasException()) {
                logger.log(Level.WARNING, "Error chunking content from " + sourceID + ": " + sourceName, chunker.getException());
    ...
    private void indexChunk(Chunk chunk, long sourceID, String sourceName, int numChunks, Optional<Language> language, Map<String, String> contentFields, boolean hasNext) throws IngesterException {
        Map<String, Object> fields = new HashMap<>(contentFields);
        String chunkId = Server.getChunkIdString(sourceID, numChunks);
        fields.put(Server.Schema.ID.toString(), chunkId);
        fields.put(Server.Schema.CHUNK_SIZE.toString(), String.valueOf(chunk.getBaseChunkLength()));
